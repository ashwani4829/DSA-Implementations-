...
struct Book library[100]; int count = 0;

⚙️ Main Functionalities

Add Book User inputs book details and adds them to the array.

Display Books Displays all the book records in tabular form.

Search Book Linear Search: search by title or author. Binary Search: when data is sorted by title.

Sort Books Sort by Title / Author / ID using any sorting algorithm (Bubble Sort, Selection Sort, etc.)
...
Searching Linear / Binary Search O(n) / O(log n) Sorting Bubble / Selection / Quick Sort O(n²) / O(n log n) Data Storage Array of Structures O(n)
